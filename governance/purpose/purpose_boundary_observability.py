"""Aggregate purpose boundary observability for governor attachment."""
...
@dataclass
class PurposeBoundaryObservability:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "disclaimer": "purpose_boundary_observational_only",
        }


def observe_purpose_boundary(
    text: str,
    *,
...
) -> PurposeBoundaryObservability:
    core = PurposeBoundaryCore()
    verdict = core.evaluate(text, purpose_id=purpose_id, runtime_id=runtime_id, scope=scope)
    prov = PurposeProvenance().validate(provenance_payload, sovereign_id=runtime_id)
    containment = MotivationalContainment().evaluate(text)
    integrity = PurposeIntegrityMonitor().check(text)
    ...
    return PurposeBoundaryObservability(
        advisory_only=True,
