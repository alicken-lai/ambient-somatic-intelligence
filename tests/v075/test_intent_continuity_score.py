"""Area 9–10: CognitiveIntentContinuityScore gate."""
...
def test_gate_threshold() -> None:
    assert INTENT_CONTINUITY_GATE_THRESHOLD == 0.90


def test_default_score_passes_gate() -> None:
    report = evaluate_cognitive_intent_continuity()
    assert report.intent_continuity_score >= 0.90
    assert report.gate_pass is True


def test_clean_evidence_passes() -> None:
    ev = CognitiveIntentContinuityAttentionEvidence(
    ...
    report = evaluate_cognitive_intent_continuity(ev)
    assert report.intent_continuity_score >= 0.90
    assert report.gate_pass is True
