"""Area 9–10: CognitiveRealityAlignmentScore gate."""
...
def test_gate_threshold() -> None:
    assert REALITY_ALIGNMENT_GATE_THRESHOLD == 0.90


def test_default_score_passes_gate() -> None:
    report = evaluate_cognitive_reality_alignment()
    assert report.reality_alignment_score >= 0.90
    assert report.gate_pass is True


def test_clean_evidence_passes() -> None:
    ev = CognitiveRealityAlignmentAttentionEvidence(
    ...
    report = evaluate_cognitive_reality_alignment(ev)
    assert report.reality_alignment_score >= 0.90
    assert report.gate_pass is True
