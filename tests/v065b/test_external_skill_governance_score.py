"""Area 9–10: ExternalSkillGovernanceScore gate."""
...
def test_gate_threshold() -> None:
    assert EXTERNAL_SKILL_GATE_THRESHOLD == 0.90


def test_clean_evidence_passes() -> None:
    ev = ExternalSkillAttentionEvidence(
    ...
    report = evaluate_external_skill_governance(ev)
    assert report.external_skill_score >= 0.90
    assert report.gate_pass is True


def test_default_forecaster_score() -> None:
    report = evaluate_external_skill_governance()
    assert report.external_skill_score >= 0.85
