"""Area 9–10: CognitiveTemporalContinuityScore gate."""
...
def test_gate_threshold() -> None:
    assert TEMPORAL_CONTINUITY_GATE_THRESHOLD == 0.90


def test_default_score_passes_gate() -> None:
    report = evaluate_cognitive_temporal_continuity()
    assert report.temporal_continuity_score >= 0.90
    assert report.gate_pass is True


def test_clean_evidence_passes() -> None:
    ev = CognitiveTemporalContinuityAttentionEvidence(
    ...
    report = evaluate_cognitive_temporal_continuity(ev)
    assert report.temporal_continuity_score >= 0.90
    assert report.gate_pass is True
