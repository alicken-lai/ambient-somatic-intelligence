"""Area 9–10: CognitivePurposeBoundaryScore gate."""
...
def test_gate_threshold() -> None:
    assert PURPOSE_BOUNDARY_GATE_THRESHOLD == 0.90


def test_default_score_passes_gate() -> None:
    report = evaluate_cognitive_purpose_boundary()
    assert report.purpose_boundary_score >= 0.90
    assert report.gate_pass is True


def test_clean_evidence_passes() -> None:
    ev = CognitivePurposeBoundaryAttentionEvidence(
    ...
    report = evaluate_cognitive_purpose_boundary(ev)
    assert report.purpose_boundary_score >= 0.90
    assert report.gate_pass is True
