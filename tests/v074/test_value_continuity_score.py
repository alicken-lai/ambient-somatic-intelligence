"""Area 9–10: CognitiveValueContinuityScore gate."""
...
def test_gate_threshold() -> None:
    assert VALUE_CONTINUITY_GATE_THRESHOLD == 0.90


def test_default_score_passes_gate() -> None:
    report = evaluate_cognitive_value_continuity()
    assert report.value_continuity_score >= 0.90
    assert report.gate_pass is True


def test_clean_evidence_passes() -> None:
    ev = CognitiveValueContinuityAttentionEvidence(
    ...
    report = evaluate_cognitive_value_continuity(ev)
    assert report.value_continuity_score >= 0.90
    assert report.gate_pass is True
