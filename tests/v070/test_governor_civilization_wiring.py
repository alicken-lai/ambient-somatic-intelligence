"""Area 10: Governor civilization observability + simulations."""

from pathlib import Path

from attention.core.attention_target import AttentionTarget
from governance.cognition.cognitive_governor import CognitiveGovernor
from v070_runtime.simulations import run_simulations, write_timeseries


def test_governor_attaches_civilization_observability() -> None:
    gov = CognitiveGovernor()
    t = AttentionTarget(
        source_domain="telemetry",
        signal_type="civilization-wire-test",
        raw_value=0.6,
    )
    d = gov.govern_target(t)
    assert d.civilization_observability is not None
    assert d.civilization_observability.get("advisory_only") is True
    before = d.accepted
...
def test_simulations_and_timeseries(tmp_path: Path) -> None:
    ...
    assert data["gate_pass"] or data["civilization_score"] >= 0.88
    assert len(data["stress_scenarios"]) == 7
    assert len(data["horizons"]) == 5
    out = tmp_path / "inter_sovereign_timeseries.json"
