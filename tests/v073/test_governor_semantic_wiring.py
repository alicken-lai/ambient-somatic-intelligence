"""Area 10: Governor semantic observability + simulations."""

from pathlib import Path

from attention.core.attention_target import AttentionTarget
from governance.cognition.cognitive_governor import CognitiveGovernor
from v073_runtime.simulations import run_simulations, write_timeseries


def test_governor_attaches_semantic_continuity_observability() -> None:
    gov = CognitiveGovernor()
    t = AttentionTarget(
        source_domain="telemetry",
        signal_type="semantic-wire-test",
        raw_value=0.6,
    )
    ...
    assert d.semantic_continuity_observability.get("advisory_only") is True
    before = d.accepted
...
def test_simulations_and_timeseries(tmp_path: Path) -> None:
    ...
    assert data["gate_pass"] or data["meaning_continuity_score"] >= 0.88
    assert len(data["stress_scenarios"]) == 7
    assert len(data["horizons"]) == 6
    out = tmp_path / "civilization_semantic_timeseries.json"
