"""Area 9–10: MetaCognitiveStabilityScore gate + simulations."""

from pathlib import Path

from observability.v064.metacognitive_stability_score import (
    METACOGNITIVE_GATE_THRESHOLD,
    MetaCognitiveAttentionEvidence,
    evaluate_metacognitive_stability,
    evidence_from_metacognitive_forecaster,
)
from v064_runtime.simulations import WINDOWS, run_all_windows, write_timeseries


def test_gate_threshold_090() -> None:
    assert METACOGNITIVE_GATE_THRESHOLD == 0.90


def test_clean_evidence_passes() -> None:
    ev = MetaCognitiveAttentionEvidence(
    ...
    report = evaluate_metacognitive_stability(ev)
    assert report.metacognition_score >= 0.90
    assert report.gate_pass is True


def test_forecaster_evidence(meta_forecaster, meta_bridge) -> None:
    ev = evidence_from_metacognitive_forecaster(
        meta_forecaster, bridge=meta_bridge
    )
    report = evaluate_metacognitive_stability(
        ev, forecaster=meta_forecaster, bridge=meta_bridge
    )
    assert report.metacognition_score >= 0.85
...
def test_run_all_windows_gate() -> None:
    ...
    for w in data["windows"].values():
        assert w["metacognition_score"] >= 0.85


def test_write_timeseries(tmp_path: Path) -> None:
    out = tmp_path / "cognition_quality_timeseries.json"
