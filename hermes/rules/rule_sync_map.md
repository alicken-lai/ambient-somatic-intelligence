...
|-- .cursor/rules/provider-subagent.mdc  - Cursor provider/subagent workspace rule (alwaysApply)
...
| Freeze / replay | section 6 in canonical | Freeze section | Hard prohibitions in codex | No self-verify + replay respect |
| Git safety | All Cursor files | copilot-instructions | AGENTS + codex | Escalation triggers |
| Provider / subagent division | `provider-subagent.mdc` + cursor export | Copilot Hermes Subagents section | Codex Hermes Subagents section | Antigravity Cross-IDE Hermes subagents |
| Traditional Chinese communication | cursor export + ambient-os.mdc | copilot Communication section | codex workflow | antigravity Communication |
...
2. [ ] Port to `cursor_rules_export.md`, `.cursor/rules/ambient-os.mdc`, and specialized Cursor workspace rules such as `.cursor/rules/provider-subagent.mdc`
...
4. Keep workspace [`provider-subagent.mdc`](../../.cursor/rules/provider-subagent.mdc) for Provider/Subagent orchestration defaults
